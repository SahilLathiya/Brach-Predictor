...
template <std::size_t LEN_GLOBAL>
struct Bank {
  int geometry; // This is predefined by GEOMETRICS and never changes, so does not count into total number of bits used.
...
template <std::size_t NUM_BANKS, std::size_t BIMODAL_SIZE, std::size_t LEN_BIMODAL, std::size_t LEN_GLOBAL, std::size_t LEN_TAG, std::size_t LEN_COUNTS,
          std::size_t MIN_HISTORY_LEN, std::size_t MAX_HISTORY_LEN>
struct Tage {
  ...
  Bank<LEN_GLOBAL> tageBank[NUM_BANKS];
  ...
  constexpr size_t BIMODAL_INDEX(const size_t pc) { return pc % BIMODAL_SIZE; }

  Tage()
  {
    // Initialize GEOMETRICS array
    double inv_n_1 = static_cast<double>(1) / static_cast<double>(NUM_BANKS - 1);
    double ratio = pow(static_cast<double>(MAX_HISTORY_LEN / MIN_HISTORY_LEN), inv_n_1);
    std::cout << "Ratio: " << ratio << "\n";

    std::cout << "Component sizes: ";
    GEOMETRICS[0] = MIN_HISTORY_LEN;
    std::cout << (int)GEOMETRICS[0] << " ";
    for (size_t i = 1; i < NUM_BANKS; i++) {
      GEOMETRICS[i] = (GEOMETRICS[i - 1] * ratio + 0.5f);
      std::cout << (int)GEOMETRICS[i] << " ";
    }
    // for (size_t i = 1; i < NUM_BANKS; i++) {
    //   GEOMETRICS[i] = (GEOMETRICS[0] * pow(static_cast<double>ratio,static_cast<double>i) + 0.5f);
    //   std::cout << (int)GEOMETRICS[i] << " ";
    // }
    std::cout << std::endl;
  }
  ...
  uint8_t t_getBimodalPrediction(uint32_t pc) { return (uint8_t)((t_bimodalPredictor[BIMODAL_INDEX(pc)] >= (1 << (LEN_BIMODAL / 2))) ? TAKEN : NOTTAKEN); }
  ...
  uint64_t getGlobalIndex(uint64_t pc, int bankIdx)
  {
    ...
    if (tageBank[bankIdx].geometry >= 16)
      index = pc ^ (pc >> ((LEN_GLOBAL - (NUM_BANKS - bankIdx - 1)))) ^ tageBank[bankIdx].indexCompressed.compressed ^ F(t_pathHistory, 16, bankIdx);

    else
      index = pc ^ (pc >> (LEN_GLOBAL - NUM_BANKS + bankIdx + 1)) ^ tageBank[bankIdx].indexCompressed.compressed
  ...
  uint8_t tage_predict(uint64_t pc)
  {
    ...
    if (primaryBank < NUM_BANKS) {
      if (alternateBank < NUM_BANKS) {
        alternatePrediction = ((tageBank[alternateBank].entry[bankGlobalIndex[alternateBank]].saturateCounter >= 0) ? TAKEN : NOTTAKEN);
      } else {
      ...
      if ((tageBank[primaryBank].entry[bankGlobalIndex[primaryBank]].saturateCounter != 0)
          ...
          || (tageBank[primaryBank].entry[bankGlobalIndex[primaryBank]].usefulness != 0) || (useAlternate < 8)) {
  ...
  void tage_train(uint64_t pc, uint8_t outcome)
  {
    ...
    if (primaryBank < NUM_BANKS) {
      ...
      int isNewAllocated = (entry.saturateCounter == -1 || entry.saturateCounter == 0) && (entry.usefulness == 0);
    ...
    if ((lastPrediction != alternatePrediction)) {
      updateSaturateMinMax(&(tageBank[primaryBank].entry[bankGlobalIndex[primaryBank]].usefulness), (lastPrediction == outcome), 0, 3);
    }
